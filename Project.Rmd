...
Note that the same preprocessing was done for all datasets (training, cross-validation and test).
...
The overall approach used was to create an odd number of independent models and then to use a voting method for the final prediction for each test record.  For this project, three independent models were created as described below.
...
The predictions for the cross-validation dataset for the three models above were combined into a data frame.  A voting function was developed to select an overall prediction that was based on the predictions from the tree models.  Each model was given one vote, and for each sample, whichever model received the largest number of votes won the vote and that answer was selected as the prediction for that sample.

For the combined model, after performing the voting on the cross-validation predictions, the confusion matrix indicates an accuracy of 96.25%, so a 3.75% out of sample error is expected for the voted results.

```{r combinedPredictors, cache=TRUE}
# vote the prediction results from the separate models to create the final prediction
...
The combined model was used to make predictions for the 20 sample records contined in the test dataset.  The predicted values of the voted results are presented below and were submitted for the project.  All 20 predictions for the test dataset were correct, which is consistent with the expected out of sample error rate of 3.75% (for which less than 1 in 20 errors would be expected).
