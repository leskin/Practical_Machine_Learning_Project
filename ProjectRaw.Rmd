...
##table(pmlTrainingData$X)
#table(pmlTrainingData$user_name)
...
#preTrainingSet <- trainingSet[,-c(1,2,3,4,5)]
#preCrossValidationSet <- crossValidationSet[,-c(1,2,3,4,5)]
```
...
#sum(is.na(preTrainingSet))
#badTrainRecords <- preTrainingSet[!complete.cases(preTrainingSet),]
#dim(badTrainRecords)
#preTrainingSet <- preTrainingSet[complete.cases(preTrainingSet),]
##head(badRecords)
#preCrossValidationSet <- preCrossValidationSet[complete.cases(preCrossValidationSet),]
#badTestRecords <- preTestingSet[!complete.cases(preTestingSet),]
#dim(badTestRecords)
```
...
library(caret)
set.seed(12345)
# decided not to do PCA since results are worse
#preProc <- preProcess(log10(subset(preTrainingSet,select=-c(classe))+1),method="pca",pcaComp=2) 
#preProc <- preProcess(subset(preTrainingSet,select=-c(classe)),method="pca",pcaComp=7) 
#trainPC <- predict(preProc, log10(subset(preTrainingSet,select=-c(classe))+1))
#testPC <- predict(preProc, log10(subset(preCrossValidationSet,select=-c(classe))+1))
#testPC <- predict(preProc, subset(preCrossValidationSet,select=-c(classe)))

# tree model fit
# using PCA
#treePCFit <- train(preTrainingSet$classe ~ .,method="rpart",data=trainPC)
#treePCPrediction <- predict(treePCFit,testPC)
#confusionMatrix(preCrossValidationSet$classe, treePCPrediction)
